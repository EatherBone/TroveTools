function initializeVenturineCalculator(container) {
    ...
    const blocks = {
        v2s: container.querySelector("#venturineInputBlockModal"),
        s2v: container.querySelector("#signetInputBlockModal"),
        continue: container.querySelector("#continueInputBlockModal"),
    ...
    const calculateVenturine = () => {
        ...
        try {
            ...
            if (selectedMode === "v2s") {
                ...
                message = `You can craft <strong>${i}</strong> Signets with <strong>${v}</strong> Venturine.`;
                ...
                message = `You need <strong>${total}</strong> Venturine to craft <strong>${s}</strong> Signets.`;
                ...
                message = `You'll need <strong>${total}</strong> Venturine to craft <strong>${desired}</strong> more Signets.`;
                ...
                message = `To craft <strong>${desired}</strong> Signets you'll need: <strong>≈${needed}</strong> Venturine`;
            }
            resultDiv.innerHTML = `<div class="result-line final-result"><span class="result-value" style="text-align: center; width: 100%;">${message}</span></div>`;
        } catch (e) {
